        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0 14px", flexWrap: "wrap", gap: "8px" }}>
          ...
          <div style={{ display: "flex", gap: "7px", alignItems: "center" }}>
            ...
            <div ref={sortR} style={{ position: "relative" }}>
              ...
              {sortOpen && <div style={{ position: "absolute", top: "110%", right: 0, background: t.card, border: `1px solid ${t.brd}`, borderRadius: "10px", boxShadow: t.shd, zIndex: 100, overflow: "hidden", minWidth: "120px" }}>
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: viewMode === "grid" ? "repeat(auto-fill,minmax(min(100%,280px),1fr))" : "1fr", gap: "10px", paddingBottom: "80px" }}>
            {sorted.map((s, i) => {
              ...
              return (
                <div key={s.id} className="sc" style={{ background: t.card, borderRadius: "14px", overflow: "hidden", border: `1px solid ${t.brd}`, boxShadow: t.shd, animationDelay: `${Math.min(i, 20) * 0.02}s` }}>
...
function Slot({ songs, t }) {
  ...
  useEffect(() => {
    let to; const st = Date.now(), dur = 2600;
    const tick = () => { const p = Math.min((Date.now() - st) / dur, 1); setIdx((i) => (i + 1) % songs.length); if (p < 1) to = setTimeout(tick, 50 + Math.pow(p, 2.5) * 600); };
    to = setTimeout(tick, 50); return () => clearTimeout(to);
  }, [songs.length]);
  const s = songs[idx % songs.length]; if (!s) return null;
  return (
