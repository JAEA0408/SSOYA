...
export default function SSOYA() {
  ...
  // 노래 추가 모달: 제목+아티스트 입력 시 커버 자동검색 (0.8초 딜레이)
  useEffect(() => {
  ...
  const handleEditCoverSearch = async () => {
    if (!editData.title && !editData.artist) return;
  ...
  // 테마
  const t = dark
    ? { bg: "#0f0f1a", card: "#1a1a2e", text: "#e2e8f0", sub: "#94a3b8", brd: "#2d2d44", acc: "#3b82f6", inBg: "#1a1a2e", inBrd: "#2d2d44", shd: "0 2px 12px rgba(0,0,0,0.4)", mod: "rgba(0,0,0,0.75)", acBg: "#1e1e32", acHover: "#252542", editBar: "#1a2540", editBtn: "#1e3a5f" }
    : { bg: "#f1f5f9", card: "#ffffff", text: "#1e293b", sub: "#64748b", brd: "#e2e8f0", acc: "#3b82f6", inBg: "#f8fafc", inBrd: "#d1d5db", shd: "0 2px 12px rgba(0,0,0,0.08)", mod: "rgba(0,0,0,0.5)", acBg: "#ffffff", acHover: "#f1f5f9", editBar: "#eff6ff", editBtn: "#dbeafe" };
                    ...
                    <div style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", gap: "12px", marginBottom: "10px" }}>
                        ...
                        <div style={{ position: "relative", width: "82px", minWidth: "82px", height: "82px", background: editBg, borderRadius: "10px", flexShrink: 0, overflow: "hidden" }}>
                          {editData.albumCover && (
                            <button onClick={() => { setEditData((p) => ({ ...p, albumCover: "" })); setEditCoverResults([]); setEditCoverOpen(false); }} style={{ position: "absolute", top: "4px", right: "4px", width: "22px", height: "22px", borderRadius: "50%", background: "rgba(239,68,68,0.85)", border: "none", color: "#fff", fontSize: "12px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
                          )}
                          <button onClick={() => { setEditCoverOpen((p) => !p); if (!editCoverOpen) setEditCoverResults([]); }} style={{ position: "absolute", bottom: "4px", right: "4px", width: "24px", height: "24px", borderRadius: "50%", background: "rgba(59,130,246,0.85)", border: "none", color: "#fff", fontSize: "14px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                        </div>
                        {/* 제목 + 아티스트 */}
                        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "7px" }}>
                          <input value={editData.title} onChange={(e) => setEditData((p) => ({ ...p, title: e.target.value }))} placeholder="제목" style={{ width: "100%", padding: "7px 10px", borderRadius: "8px", border: `1.5px solid ${t.acc}`, background: t.inBg, color: t.text, fontSize: "14px", fontWeight: 700, outline: "none" }} />
                          <input value={editData.artist} onChange={(e) => setEditData((p) => ({ ...p, artist: e.target.value }))} placeholder="아티스트" style={{ width: "100%", padding: "7px 10px", borderRadius: "8px", border: `1px solid ${t.inBrd}`, background: t.inBg, color: t.text, fontSize: "13px", outline: "none" }} />
                        </div>
                      </div>

                      {/* 커버 편집 패널 */}
                      {editCoverOpen && (
                        <div style={{ marginBottom: "10px", padding: "10px", background: t.inBg, borderRadius: "10px", border: `1px solid ${t.inBrd}` }}>
                          <div style={{ display: "flex", gap: "6px", marginBottom: editCoverResults.length > 0 ? "8px" : 0 }}>
                            <input value={editData.albumCover} onChange={(e) => setEditData((p) => ({ ...p, albumCover: e.target.value }))} placeholder="앨범커버 URL 직접 입력" style={{ flex: 1, padding: "7px 10px", borderRadius: "8px", border: `1px solid ${t.inBrd}`, background: t.card, color: t.text, fontSize: "12px", outline: "none" }} />
                            <button onClick={handleEditCoverSearch} disabled={editCoverLoading} style={{ padding: "7px 12px", borderRadius: "8px", background: t.acc, color: "#fff", border: "none", fontSize: "12px", fontWeight: 600, cursor: editCoverLoading ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}>{editCoverLoading ? "검색중..." : "🔍 검색"}</button>
                          </div>
                          {editCoverResults.length > 0 && (
                            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                              {editCoverResults.map((r, idx) => (
                                <div key={idx} onClick={() => { setEditData((p) => ({ ...p, albumCover: r.cover })); setEditCoverResults([]); setEditCoverOpen(false); }} style={{ cursor: "pointer", width: "58px" }}>
                                  <img src={r.small} alt="" style={{ width: "58px", height: "58px", borderRadius: "8px", objectFit: "cover", border: editData.albumCover === r.cover ? `2.5px solid ${t.acc}` : `1.5px solid ${t.brd}` }} />
                                  <div style={{ fontSize: "9px", color: t.sub, marginTop: "2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.label}</div>
                                </div>
                              ))}
                            </div>
                          )}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "7px", marginBottom: "10px" }}>
                        ...
                        <input type="number" value={editData.starCount} min="0" onChange={(e) => { const n = parseInt(e.target.value, 10); setEditData((p) => ({ ...p, starCount: isNaN(n) || n < 0 ? 0 : n })); }} style={{ width: "60px", padding: "4px", borderRadius: "8px", border: `1px solid ${t.inBrd}`, background: t.inBg, color: t.text, fontSize: "15px", fontWeight: 700, textAlign: "center", outline: "none" }} />
        ...
        <div onClick={(e) => { if (e.target === e.currentTarget) closeAddModal(); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: "20px" }}>
          <div style={{ background: t.card, borderRadius: "20px", padding: "28px 24px", maxWidth: "420px", width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: `1px solid ${t.brd}`, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ marginBottom: "10px" }}>
              {addCoverLoading ? (
                <div style={{ padding: "16px", textAlign: "center", color: t.sub, fontSize: "13px", background: t.inBg, borderRadius: "10px", border: `1px solid ${t.inBrd}` }}>🔍 iTunes에서 커버 검색 중...</div>
              ) : addCoverResults.length > 0 ? (
                <div style={{ padding: "10px", background: t.inBg, borderRadius: "10px", border: `1px solid ${t.inBrd}` }}>
                  <p style={{ fontSize: "11px", color: t.sub, marginBottom: "8px", fontWeight: 600 }}>클릭해서 선택 (첫 번째가 자동 선택됨)</p>
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "8px" }}>
                    {addCoverResults.map((r, idx) => (
                      <div key={idx} onClick={() => setAddForm((p) => ({ ...p, albumCover: p.albumCover === r.cover ? "" : r.cover }))} style={{ cursor: "pointer", width: "68px", position: "relative" }}>
                        <img src={r.small} alt="" style={{ width: "68px", height: "68px", borderRadius: "10px", objectFit: "cover", border: addForm.albumCover === r.cover ? `3px solid ${t.acc}` : `1.5px solid ${t.brd}`, transition: "border .15s" }} />
                        {addForm.albumCover === r.cover && (
                        ...
                        <div style={{ fontSize: "9px", color: t.sub, marginTop: "3px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.label}</div>
                      </div>
                    ))}
                  </div>
                  <button onClick={handleAddCoverSearchManual} style={{ fontSize: "11px", color: t.acc, background: "none", border: "none", cursor: "pointer", fontWeight: 600, padding: 0 }}>🔄 다시 검색</button>
                </div>
              ) : addForm.title && addForm.artist ? (
                <div style={{ padding: "12px", textAlign: "center", color: t.sub, fontSize: "12px", background: t.inBg, borderRadius: "10px", border: `1px solid ${t.inBrd}` }}>
                  iTunes에서 커버를 찾지 못했어요.
                  <button onClick={handleAddCoverSearchManual} style={{ marginLeft: "6px", color: t.acc, background: "none", border: "none", cursor: "pointer", fontSize: "12px", fontWeight: 600 }}>재시도</button>
                </div>
              ) : (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "22px" }}>
              ...
              <input type="number" value={addForm.starCount} min="0" onChange={(e) => { const n = parseInt(e.target.value, 10); setAddForm((p) => ({ ...p, starCount: isNaN(n) || n < 0 ? 0 : n })); }} style={{ width: "65px", height: "34px", borderRadius: "10px", border: `1px solid ${t.inBrd}`, background: t.inBg, color: t.text, fontSize: "16px", fontWeight: 700, textAlign: "center", outline: "none" }} />
